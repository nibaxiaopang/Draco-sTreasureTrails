//
//  TreasurePravicyViewController.swift
...
class TreasurePravicyViewController: UIViewController , WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate{

    let KdracosPrivacyPoliceUrl = "https://www.termsfeed.com/live/ef81c634-8358-4c86-a532-3d295dd7b2e6"
    
    @IBOutlet weak var indicatorView: UIActivityIndicatorView!
    @IBOutlet weak var dracosWebView: WKWebView!
    ...
    override func viewDidLoad() {
        ...
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.dracosGetUserDefaultKey())
        initSubViews()
        initNavView()
        dracosInitWebView()
        dracosStartLoadWebView()
    }
    ...
    */
    ...
    private func initSubViews() {
        dracosWebView.scrollView.contentInsetAdjustmentBehavior = .never
        view.backgroundColor = .black
        dracosWebView.backgroundColor = .black
        dracosWebView.isOpaque = false
        dracosWebView.scrollView.backgroundColor = .black
        indicatorView.hidesWhenStopped = true
    }
    
    private func initNavView() {
        guard let url = url, !url.isEmpty else {
            dracosWebView.scrollView.contentInsetAdjustmentBehavior = .automatic
            return
        }
    ...
    private func dracosInitWebView() {
        guard let confData = privacyData, confData.count > 7 else { return }
        
        let userContentC = dracosWebView.configuration.userContentController
        ...
        dracosWebView.navigationDelegate = self
        dracosWebView.uiDelegate = self
    }
    
    
    private func dracosStartLoadWebView() {
        let urlStr = url ?? KdracosPrivacyPoliceUrl
        guard let url = URL(string: urlStr) else { return }
        
        indicatorView.startAnimating()
        let request = URLRequest(url: url)
        dracosWebView.load(request)
    }
    
    private func dracosReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "TreasurePravicyViewController") as? TreasurePravicyViewController {
                adView.url = adurl
                adView.backAction = { [weak self] in
                    let close = "window.closeGame();"
                    self?.dracosWebView.evaluateJavaScript(close, completionHandler: nil)
                }
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                dracosSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                dracosReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        dracosSendEvent(tName, values: [tName: data])
                    }
                } else {
                    dracosSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                dracosAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    dracosAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = dracosJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = dracosJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    dracosSendEvents(withParams: evParams)
                }
            }
        }
    }
